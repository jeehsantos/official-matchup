...
export default function Auth() {
  ...
  const handleLogin = async (data: LoginFormData) => {
    ...
    if (error) {
      toast({
        variant: "destructive",
        title: "Login failed",
        description: error.message === "Invalid login credentials" ? "Incorrect email or password. Please try again." : error.message
      });
    } else if (role) {
      // Check for stored redirect path from before auth
      const redirectPath = localStorage.getItem('redirectAfterAuth');
  ...
  const handleSignUp = async (data: SignUpFormData) => {
    ...
    setIsSubmitting(false);
    if (error) {
      const friendlyMessage = error.message.includes("already registered") ? "This email is already registered. Please log in instead." : error.message;
      toast({
        variant: "destructive",
  ...
  return <PublicLayout showBack={false} showFooter={false}>
      ...
      {showForgotPassword ? <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="font-display">Reset Password</CardTitle>
            <CardDescription>
              {resetSent ? "Check your email for a password reset link" : "Enter your email to receive a password reset link"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {!resetSent ? <div className="space-y-4">
                <div>
              ...
              </div> : <Button className="w-full btn-athletic" onClick={() => {
            setShowForgotPassword(false);
            setResetEmail("");
            setResetSent(false);
          }}>
                ...
                <Form {...loginForm}>
                  <form onSubmit={loginForm.handleSubmit(handleLogin)} className="space-y-4">
                    ...
                    <Button type="button" variant="outline" className="w-full" onClick={handleGoogleSignIn} disabled={isSubmitting || isGoogleLoading}>
                      {isGoogleLoading ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <svg className="h-4 w-4 mr-2" viewBox="0 0 24 24">
